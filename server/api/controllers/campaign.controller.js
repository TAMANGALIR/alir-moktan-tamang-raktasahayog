...
exports.getPublicCampaignById = exports.registerGuest = exports.updateRegistrationStatus = exports.checkAndCompleteCampaigns = exports.checkRegistrationStatus = exports.registerForCampaign = exports.getPublicCampaigns = exports.updateCampaignStatus = exports.getAllCampaignsForAdmin = exports.deleteCampaign = exports.getCampaignDetails = exports.updateCampaign = exports.getOrgCampaigns = exports.createCampaign = void 0;
...
const createCampaign = async (req, res) => {
    try {
        ...
        const campaign = await prisma.campaign.create({
            ...
            }, // Cast to any to bypass stale Prisma types until restart
            include: { organizer: true } // Include organizer for notification details
        });
        // Notify Admins
        (0, socket_utils_1.notifyAdmins)('CAMPAIGN_CREATED', {
            message: `New Campaign Request: "${title}" by ${campaign.organizer?.name}`,
...
const getOrgCampaigns = async (req, res) => {
    try {
        // Robust lookup: Get Organization ID from DB using User ID
        let organizationId = req.user?.organizationId;
        if (!organizationId && req.user?.id) {
...
exports.getOrgCampaigns = getOrgCampaigns;
...
const updateCampaign = async (req, res) => {
    try {
        ...
        const campaignAny = campaign; // Cast for stale types
        let newStatus = campaign.status;
        let newRejectionReason = campaignAny.rejectionReason;
        if (campaign.status === client_1.CampaignStatus.REJECTED) {
            newStatus = client_1.CampaignStatus.PENDING;
        ...
        (0, socket_utils_1.notifyAdmins)('CAMPAIGN_UPDATED', {
            message: `Campaign Updated: "${title}" by ${updatedCampaign.organizer?.name}`,
...
const getAllCampaignsForAdmin = async (req, res) => {
    try {
        ...
        const whereClause = {};
        // Status Filter
        if (status !== 'ALL' && Object.values(client_1.CampaignStatus).includes(status)) {
...
exports.getAllCampaignsForAdmin = getAllCampaignsForAdmin;
...
const updateCampaignStatus = async (req, res) => {
    try {
        ...
        if (status === client_1.CampaignStatus.REJECTED && !reason) {
        ...
        const campaign = await prisma.campaign.update({
            ...
            data: {
                status,
                rejectionReason: status === client_1.CampaignStatus.REJECTED ? reason : null
            },
            include: { organizer: { include: { admins: true } } }
        });
        // Auto-refund on rejection if payment was completed
        if (status === client_1.CampaignStatus.REJECTED) {
        ...
        const campaignAny = campaign;
        if (campaignAny.organizer?.admins) {
            const message = status === client_1.CampaignStatus.APPROVED
                ? `Your campaign "${campaign.title}" has been APPROVED!`
                : `Your campaign "${campaign.title}" has been REJECTED. Reason: ${reason}`;
            const type = status === client_1.CampaignStatus.APPROVED ? 'GENERAL' : 'REQUEST_ALERT';
...
async function processNotifications(admins, message, type, campaignId) {
    // 1. Bulk Create in DB
    ...
    admins.forEach(admin => {
        (0, socket_utils_1.sendNotification)(admin.userId, 'CAMPAIGN_UPDATE', {
            message,
            campaignId,
...
const getPublicCampaigns = async (req, res) => {
    try {
        ...
        const campaignsWithDonations = await Promise.all(campaigns.map(async (campaign) => {
...
exports.registerForCampaign = registerForCampaign;
...
const checkAndCompleteCampaigns = async () => {
    try {
        const today = new Date();
...
exports.checkAndCompleteCampaigns = checkAndCompleteCampaigns;
// ------------------------------------------------------------------
...
const updateRegistrationStatus = async (req, res) => {
    try {
        ...
        if (!Object.values(client_2.RegistrationStatus).includes(status)) {
            return res.status(400).json({ message: 'Invalid status' });
        }
        ...
        if (status === client_2.RegistrationStatus.DONATED) {
            ...
            if (registration.userId && registration.user?.donorProfile) {
                ...
                if (registration.user.email) {
                    await (0, email_utils_1.sendEmail)(registration.user.email, 'Thank You for Your Donation - Raktasahayog', `<div style="font-family: Arial, sans-serif; color: #333;">
    ...
    try {
        ...
        const registration = await prisma.campaignRegistration.create({
            data: {
                ...
                guestPhone: phone,
                guestEmail: email,
                status: status || client_2.RegistrationStatus.REGISTERED
            }
        });
        res.json(registration);
    }
...
exports.getPublicCampaignById = getPublicCampaignById;
