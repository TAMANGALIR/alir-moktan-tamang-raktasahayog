...
exports.markUnitAsUsed = exports.updateInventory = exports.addDonation = exports.getInventoryDetails = exports.getMyRegionInventory = void 0;
...
const normalizeBloodGroup = (bg) => {
    ...
    if (Object.values(client_1.BloodGroup).includes(bg))
        return bg;
    return map[bg] || null;
};
// Get inventory for a specific admin's Organization
const getMyRegionInventory = async (req, res) => {
    try {
...
exports.getMyRegionInventory = getMyRegionInventory;
    ...
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const existingInv = await tx.bloodInventory.findFirst({
                where: { organizationId: orgId, bloodGroup: bloodGroup }
            });
            if (existingInv) {
...
const markUnitAsUsed = async (req, res) => {
    try {
        ...
        unitsToProcess.forEach(u => {
            if (u.donor?.email)
                uniqueDonors.set(u.donor.email, u.donor);
        });
        const { sendEmail } = require('../utils/email.utils');
        for (const [email, donor] of uniqueDonors) {
            await sendEmail(email, 'Your Donation Saved a Life! - Raktasahayog', `<div style="font-family: Arial, sans-serif; color: #333;">
...
exports.markUnitAsUsed = markUnitAsUsed;
