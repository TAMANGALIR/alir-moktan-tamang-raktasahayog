...
exports.rebroadcastEmergency = exports.updateEmergencyStatus = exports.getMyEmergencyRequests = exports.respondToEmergency = exports.getActiveEmergencies = exports.createEmergencyRequest = void 0;
...
const createEmergencyRequest = async (req, res) => {
    try {
        const { bloodGroup, unitsNeeded, urgencyLevel, location, latitude, longitude, hospitalName, contactPerson, contactPhone, radiusKm, message, expiresAt } = req.body;
        // Get organization ID
        ...
        const eligibleDonors = await findEligibleDonors(bloodGroup, parseFloat(latitude), parseFloat(longitude), parseFloat(radiusKm || '10'));
        ...
        const notificationPromises = eligibleDonors.map(donor => {
            ...
            (0, socket_utils_1.sendNotification)(donor.user.id, 'EMERGENCY', {
                message,
...
async function findEligibleDonors(bloodGroup, latitude, longitude, radiusKm) {
    ...
    try {
        ...
        const requestsWithDistance = activeRequests.map(request => {
            ...
            if (donorProfile.latitude && donorProfile.longitude) {
                distance = calculateDistance(request.latitude, request.longitude, donorProfile.latitude, donorProfile.longitude);
...
const respondToEmergency = async (req, res) => {
    try {
        ...
        if (request.status !== client_1.EmergencyStatus.ACTIVE) {
        ...
        if (status === 'ACCEPTED') {
            try {
                ...
                const adminNotifications = orgAdmins.map(admin => {
                    ...
                    (0, socket_utils_1.sendNotification)(admin.userId, 'EMERGENCY_RESPONSE', {
...
exports.getMyEmergencyRequests = getMyEmergencyRequests;
...
const rebroadcastEmergency = async (req, res) => {
    try {
        const { id } = req.params;
        let organizationId = req.user?.organizationId;
        if (!organizationId && req.user?.id) {
        ...
        const eligibleDonors = await findEligibleDonors(request.bloodGroup, request.latitude, request.longitude, request.radiusKm);
        const notificationPromises = eligibleDonors.map(donor => {
...
exports.rebroadcastEmergency = rebroadcastEmergency;
