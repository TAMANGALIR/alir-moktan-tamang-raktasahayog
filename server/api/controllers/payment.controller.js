...
exports.verifyDonationPayment = exports.initiateDonation = exports.getPaymentStatus = exports.cancelCampaign = exports.refundPayment = exports.verifyEsewaPayment = exports.initiatePayment = void 0;
const client_1 = require("@prisma/client");
const crypto_1 = __importDefault(require("crypto"));
const axios_1 = __importDefault(require("axios"));
const prisma = new client_1.PrismaClient();
const prismaAny = prisma; // Bypass stale Prisma types after migration
// ─── CONFIG ───────────────────────────────────────────────────────────
...
function generateEsewaSignature(message) {
    const hash = crypto_1.default.createHmac('sha256', ESEWA_SECRET_KEY)
        .update(message)
        .digest('base64');
    return hash;
}
...
const verifyEsewaPayment = async (req, res) => {
    try {
        ...
        if (decodedData.signed_field_names) {
            ...
            try {
                const statusResponse = await axios_1.default.get(ESEWA_STATUS_URL, {
                    params: {
                        product_code: ESEWA_PRODUCT_CODE,
...
const cancelCampaign = async (req, res) => {
    try {
        ...
        if (!['PAYMENT_PENDING', 'PENDING'].includes(campaign.status)) {
