...
exports.calculateDonorScore = calculateDonorScore;
exports.rankDonors = rankDonors;
exports.getCompatibleBloodGroups = getCompatibleBloodGroups;
 ...
 */
function calculateBloodCompatibilityScore(requestedBloodGroup, donorBloodGroup) {
 ...
 */
function calculateLocationProximityScore(recipientLocation, donorLocation) {
 ...
 */
function calculateDonorReliabilityScore(badge, totalDonations) {
 ...
 */
function calculateAvailabilityScore(nextEligibleDate, lastDonationDate) {
    const now = new Date();
    ...
    const daysUntilEligible = Math.ceil((nextEligibleDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
 ...
 */
function calculateDonorScore(requestedBloodGroup, recipientLocation, donor) {
    // Calculate individual scores (0-100 scale)
    const bloodCompatibility = calculateBloodCompatibilityScore(requestedBloodGroup, donor.bloodGroup);
    const locationProximity = calculateLocationProximityScore(recipientLocation, donor.location);
    const donorReliability = calculateDonorReliabilityScore(donor.badge, donor.totalDonations);
    const availability = calculateAvailabilityScore(donor.nextEligibleDate, donor.lastDonationDate);
    // Calculate weighted total score
    const total = ((bloodCompatibility * WEIGHTS.BLOOD_COMPATIBILITY) +
 ...
 */
function rankDonors(requestedBloodGroup, recipientLocation, donors, minScore = 0) {
    // Filter out incompatible blood groups first
    const compatibleDonors = donors.filter(donor => BLOOD_COMPATIBILITY[requestedBloodGroup].includes(donor.bloodGroup));
    // Calculate scores for all compatible donors
    const scoredDonors = compatibleDonors.map(donor => calculateDonorScore(requestedBloodGroup, recipientLocation, donor));
